...
my_names, new_seq, new_top, struct_labels, X_test, y_test =  ps.my_par(path3 + "subset_of_30_proteins.txt", fnames, 13, path2)

print(X_test.shape)
print(y_test.shape)

# Load the model
my_model = pickle.load(open(path + "RFC_PSSM.pkl","rb"))
...
plt.title('Confusion Matrix for the predicted sequences PSSM (window = 13)')
...
with open(path3 + 'true_vs_predicted_topo_RFC_PSSM.txt', 'w') as file:
