...
with open(path + "singleprot_train.txt") as f:
...
slide_window_size = 35  ### add manually the number of elements for the windows
...
############# It also works for single sequences
...
clfr = SVC(kernel = 'linear', gamma = 0.01, cache_size = 2000)

clfr.fit(X_PCA_train,y_train)


score = cross_val_score(clfr, X_PCA_train, y_train, cv = 5, verbose = True)  ####get scores for the train set
print(score)
