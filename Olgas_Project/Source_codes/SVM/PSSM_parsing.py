...
def my_par(filename, fnames, slide_window_size, path2):
    ...
    my_evolution = []
    my_names = []
    for fname in fnames:     # names of the fasta files
        my_dat = np.genfromtxt(path2 + fname, skip_header = 3, skip_footer= 5, dtype = None, usecols = range(22,42))
        my_data = my_dat/100
        #print(my_data)
        evols = my_data.tolist()            # a list of sequences, in which sequence there are lists of 20 numbers for every amino acid
        names = fname.split('.')[0]
    ...
    #### Put the topologies and the sequences on the fasta files names order and save them in new lists
    new_seq = []
    new_top = []
    ...
    #### put the list of the 20 numbers of each aminoacid in the same list for every sequence
