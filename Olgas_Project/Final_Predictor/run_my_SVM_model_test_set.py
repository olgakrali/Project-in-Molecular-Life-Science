...
path = "Datasets/"
path2 = "Final_Predictor/"

protein_ID, sequences, topology, svm2, seconstr, struct_labels = mp.my_par("test_final.txt",19)
...
my_zip = zipfile.ZipFile(path2 + "SVM_new_model.pkl.zip", "r")
my_zip.extractall(path2)
my_zip.close()

# Load the model
my_model = pickle.load(open(path2 + "SVM_new_model.pkl","rb"))
...
plt.title('Confusion Matrix for the 89 predicted sequences (window = 19)')
...
with open(path2 + 'my_final_predictions.txt', 'w') as file:
